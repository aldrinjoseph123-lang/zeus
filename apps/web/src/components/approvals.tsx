  ...
  if (status === 'NOT_REQUIRED') {
    ...
    return (
      <div className="flex flex-wrap items-center gap-2 border-t border-line bg-sunken px-4 py-2.5">
        ...
        {/* A note here means a sign-off was voided by a later price change — say so. */}
        <span className={cx('text-[12px]', record.approvalNote ? 'font-semibold text-accent-ink' : 'text-muted')}>
          {record.approvalNote ?? 'Not requested. Send it to a sales manager if it needs signing off.'}
        </span>
