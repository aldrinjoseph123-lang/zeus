...
/** The account's portal, exactly as under Settings → Portal access — here because this is where sales already are. */
function PortalCard({ accountId }: { accountId: string }) {
  const { can } = useAuth();
  const navigate = useNavigate();
  return (
    <Card>
      <CardHeader title="Portal" subtitle="What this account's people see when they sign in, and who they are. Default follows Settings → Portal access." />
      <div className="px-4 py-4">
        <PortalAccountPanel accountId={accountId} editable={can('portal', 'update')} onGrant={() => navigate('/settings/portal')} />
      </div>
    </Card>
  );
}
